...
使用枚举方式定义工具参数, 支持一个类重定义多个工具；在工具执行阶段，可以执行到对应的方法

在core模块，重点是tool的定义与注册表、执行目标与执行器；在infrastructure模块，实现Tool的不同定义方式以及对应的注册与解析方式
...
需要支持同步调用、异步调用、同步流式调用、异步流式调用，同时支持tool-calling功能

要特别处理在流式调用的场景下，获取&解析大模型返回工具调用信息的处理，转换为Messages的toolCalls，后续供Agent执行工具调用

在流式和异步调用的场景下，需要支持中断功能，除了系统内中断，还需要调用不同厂商的实现，完成中断