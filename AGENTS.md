...
- 引用类SKILL：当你认为生成的代码或功能需要增加编码示例SKILL时(例如新增核心功能、AGENT_ACTION[*]时)，参考SKILL生成规范生成对应的SKILL文件
...
> 
> 所有的修改与变更，都要先生成待评审的方案；必须得到我评审通过反馈，同时更新评审记录后再实施（无论是否在PLAN模式下工作，都必须遵守该约束）
>
> 在实施过程中，可以根据当前任务场景判断是否需要启用sub-agent并行完成，启动sub-agent前需要和我确认
